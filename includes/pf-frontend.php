	...
	class XforWC_Product_Filters_Frontend {
		...
		function make_filterjs() {
			...
			if ( !isset( $prdctfltr_global['sc_init'] ) ) {
				...
				if ( !isset( $prdctfltr_global['mobile'] )&& !is_ajax() &&  is_shop() || is_product_taxonomy() || is_search() || isset( self::$settings['shop_query']['wc_query'] ) && self::$settings['shop_query']['wc_query'] == 'product_query' ) {
			...
			switch ( self::$filter['style']['style']['type'] ) {
				...
				case 'color':
					if ( !empty( self::$filter['style']['label'] ) && self::$filter['style']['label'] == 'side' ) {
						...
						return '<span class="prdctfltr_customize_block prdctfltr_customize"><span class="prdctfltr_customize_color" style="background-color:' . Prdctfltr()->esc_color( $data['data'] ) . ';' . ( !empty( self::$filter['style']['size'] ) ? sprintf( 'width:%1$spx;height:%1$spx;', absint( self::$filter['style']['size'] ) ) : '' ) . '"></span>' . ( $cnt !== false ? ' <span class="prdctfltr_customize_count">' . $cnt . '</span>' : '' ) . ( $tip !== false ? '<span class="prdctfltr_tooltip"><span>' . wp_kses_post( $tip ) . '</span></span>' : '' ) . ( $checked !== '' ? '<input type="checkbox" value="' . esc_attr( $term_slug ) . '"' . esc_html( $checked ) . '/>' : '' ) . '<span class="prdctfltr_customization_search">' . esc_html( $term_name ) . '</span>' . wp_kses_post( $sublevel ) . '</span>';
					}
				break;

				case 'image':
					if ( !empty( self::$filter['style']['label'] ) && self::$filter['style']['label'] == 'side' ) {
						return '<span class="prdctfltr_customize_block prdctfltr_customize"' . ( !empty( self::$filter['style']['size'] ) ? sprintf( ' style="line-height:%1$spx;"', absint( self::$filter['style']['size'] ) ) : '' ) . '><span class="prdctfltr_customize_image_text"><img src="' . ( empty( $data['data'] ) ? self::$url_path . '/includes/images/pf-placeholder.gif' : esc_url( $data['data'] ) ) . '" ' . ( !empty( self::$filter['style']['size'] ) ? sprintf( 'style="height:%1$spx;" ', absint( self::$filter['style']['size'] ) ) : '' ) . '/></span>' . ( $tip !== false ? '<span class="prdctfltr_tooltip"><span>' . wp_kses_post( $tip ) . '</span></span>' : '' ) . ( $checked !== '' ? '<input type="checkbox" value="' . esc_attr( $term_slug ) . '"' . esc_html( $checked ) . '/>' : '' ) . '<span class="prdctfltr_customization_search">' . esc_html( $term_name ) . '</span><span class="prdctfltr_customize_image_text_tip">' . esc_html( $term_name ) . '</span>' . ( $cnt !== false ? ' <span class="prdctfltr_count">' . $cnt . '</span>' : '' ) . wp_kses_post( $sublevel ) . '</span>';
					}
					else {
						return '<span class="prdctfltr_customize_block prdctfltr_customize"><span class="prdctfltr_customize_image"><img src="' . ( empty( $data['data'] ) ? self::$url_path . '/includes/images/pf-placeholder.gif' : esc_url( $data['data'] ) ) . '" ' . ( !empty( self::$filter['style']['size'] ) ? sprintf( 'style="height:%1$spx;" ', absint( self::$filter['style']['size'] ) ) : '' ) . '/></span>' . ( $cnt !== false ? ' <span class="prdctfltr_customize_count">' . $cnt . '</span>' : '' ) . ( $tip !== false ? '<span class="prdctfltr_tooltip"><span>' . wp_kses_post( $tip ) . '</span></span>' : '' ) . ( $checked !== '' ? '<input type="checkbox" value="' . esc_attr( $term_slug ) . '"' . esc_html( $checked ) . '/>' : '' ) . '<span class="prdctfltr_customization_search">' . esc_html( $term_name ) . '</span>' . wp_kses_post( $sublevel ) . '</span>';
					}
				break;
...
<?php
		...
		public static function make_up_filter_700( $filter ) {
			...
			self::$filter['terms_recount'] = self::$filter['term_count'] == 'yes' && is_ajax() && is_taxonomy_hierarchical( self::$filter['taxonomy'] );
